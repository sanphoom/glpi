...
//                               return {results: data.results};
    ...
    * @return nothing
   **/
   static function displayCheckboxMatrix(array $rows, array $columns,
                                         array $meshings = array(),
                                         array $options = array()) {
      ...
      if (!empty($param['title'])) {
         ...
         echo "\t\t<th colspan='".(count($columns) + 1)."'>".$param['title']."</th>\n";
         echo "\t</tr>\n";
      }

      echo "\t<tr>\n";
      echo "\t\t<td class='skew'><div><span>&nbsp;</span></div></td>\n";
      foreach ($columns as $information) {
         echo "\t\t<td class='skew'><div><span>".$information['label']."</span></div></td>\n";
      }
      echo "\t</tr>\n";
      foreach ($rows as $row_name => $row_information) {
         echo "\t<tr class='tab_bg_1'>";
         if (!empty($row_information['field_name'])) {
            $name = $row_information['field_name'];
         } else {
            $name = $param['field_name']."[$row_name]";
         }
         echo "<td>".$row_information['label']."</td>\n";
         foreach ($columns as $col_name => $col_information) {
            if (isset($meshings[$row_name.'\\'.$col_name])) {
               $meshing = $meshings[$row_name.'\\'.$col_name];
            } else {
               $meshing = false;
            }
            $label = '';
            if ((is_bool($meshing)) || ($meshing === -1) || ($meshing === -2)) {
               if (is_bool($meshing)) {
                  $readonly = false;
                  $value    = $meshing;
               } else {
                  $readonly = true;
                  $value    = $meshing + 2; // -2 => 0 = false && -1 => 1 = true
               }
               $label = "<input type='checkbox' name='$name"."[$col_name]'";
               if ($value) {
                  $label .= 'checked';
               }
               if ($readonly) {
                  $label .= "  onclick='this.checked=!this.checked;'>";
               }
            }
            if (empty($label)) {
               if ($meshing === NULL) {
